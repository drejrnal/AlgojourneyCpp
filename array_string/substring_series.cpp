...
//leetcode 3 Given a string s, find the length of the longest substring without duplicate characters.
string longest_non_repeatable_substring(const string& str) {
	// record the character and its last position
	unordered_map<char, int> char_to_last_position;
	int left = 0, right = 0;
	int res = 0;
	int start = left;
	while (right < str.length()) {
		if (char_to_last_position.find(str[right]) !=
		    char_to_last_position.end()) {
			// left toward the last position of the character is removed from
			// the window
			int last_position = char_to_last_position[str[right]];
			for (int i = left; i <= last_position; i++) {
				char_to_last_position.erase(str[i]);
			}
			left = last_position + 1;
		}
		char_to_last_position[str[right]] = right;
		if (right - left + 1 > res) {
			res = right - left + 1;
			start = left;
		}
...
/*
 * 给你一个字符串 s ，请你返回满足以下条件的最长子字符串的长度：每个元音字母，即
 ...
 * 求解二元一次方程 lenA * numA + lenB * numB = m 其中m为value的长度,
 * numA和numB为pattern中'a'和'b'的个数， 求解lenA和lenB
...
bool matchSinglePatternAlpha(size_t len, string& value) {
	if (len == 0 || value.size() % len != 0) return false;
	string sub = value.substr(0, len);
	for (size_t i = len; i < value.size(); i += len) {
		if (value.substr(i, len) != sub) return false;
	}
	return true;
}
bool matchBiPatternAlpha(int num_a, int num_b, string& pattern, string& value) {
	...
	for (int len_a = 0; len_a * num_a <= m; len_a++) {
		if ((m - len_a * num_a) % num_b != 0) continue;
		...
		if (i == m && sub_a != sub_b) return true;
	}
	return false;
}
...
int main() {
	string s;
	while (cin >> s) {
		cout << longest_non_repeatable_substring(s) << endl;
	}
	return 0;
}
...
template < class Key, class Value,
        class Comparator = less<Value>, class Hash = hash<Key> >
...
private:
    ...
    void swim( int first, int topIndex, int holeIndex ){
        ...
        while ( holeIndex > topIndex &&
                    comparator(pq[first+parentIndex].second, pq[first+holeIndex].second ) ){
    ...
    void sink( int first, int len, int holeIndex ){
        ...
        while( (2 * holeIndex + 1) < len ){
            ...
            if( child + 1 < len &&
                            comparator(pq[first+child].second, pq[first+child+1].second) )
                child = child+1;
            if( !comparator(pq[first+holeIndex].second, pq[first+child].second) )
                break;

 ...
 *integers m, n, maxMove, startRow, startColumn, return the number of paths to
 *move the ball out of the grid boundary. Since the answer can be very large,
...
public:
	...
	long long dfs(int startRow, int startColumn, int m, int n, int movable) {
		...
		if (startColumn < 0 || startRow < 0 || startRow == m ||
		    startColumn == n) {
			return 1;
		}
		...
		for (int i = 0; i < 4; i++)
			result =
			    (result + dfs(startRow + move[i][0], startColumn + move[i][1],
			                  m, n, movable - 1)) %
			    MOD;
